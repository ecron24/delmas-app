...
# ============================================
# Script pour récupérer et appliquer toutes les migrations
# Date: 2025-11-07
# ============================================
...
git fetch origin claude/delmas-app-modifications-011CUtDKPsTnhj6EGPS6Scch
git pull origin claude/delmas-app-modifications-011CUtDKPsTnhj6EGPS6Scch

echo ""
echo "✅ Code récupéré avec succès"
echo ""

# Vérifier que tous les fichiers de migration sont présents
echo "🔍 Vérification des fichiers de migration..."
MISSING_FILES=0

if [ ! -f "supabase/migrations/20251107_create_proforma_invoice_trigger.sql" ]; then
  echo "❌ Fichier manquant: 20251107_create_proforma_invoice_trigger.sql"
  MISSING_FILES=1
fi

if [ ! -f "supabase/migrations/20251107_fix_invoice_totals_trigger.sql" ]; then
  echo "❌ Fichier manquant: 20251107_fix_invoice_totals_trigger.sql"
  MISSING_FILES=1
fi

if [ ! -f "supabase/migrations/20251107_fix_proforma_copy_products.sql" ]; then
  echo "❌ Fichier manquant: 20251107_fix_proforma_copy_products.sql"
  MISSING_FILES=1
fi

if [ ! -f "supabase/migrations/20251107_fix_sync_intervention_total.sql" ]; then
  echo "❌ Fichier manquant: 20251107_fix_sync_intervention_total.sql"
  MISSING_FILES=1
fi

if [ $MISSING_FILES -eq 1 ]; then
  echo ""
  echo "❌ ERREUR: Des fichiers de migration sont manquants"
  echo "   Vérifiez que vous êtes bien sur la branche claude/delmas-app-modifications-011CUtDKPsTnhj6EGPS6Scch"
  exit 1
fi

echo "✅ Tous les fichiers de migration sont présents"
echo ""
